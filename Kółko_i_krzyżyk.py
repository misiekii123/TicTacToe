...
# zmienna w formie listy  opisująca  pole gry, 0 - pole puste, 1 - gracz, 2 - komputer
POLE_GRY = [0, 0, 0,
            0, 0, 0,
            0, 0, 0]
...
WYGRANA = False   # ustawienie zmiennej na false pozwala włączyć gre


# rysowanie planszy gry 3x3
def rysuj_plansze():
...
def rysuj_pole_gry():
    for i in range(0, 3):
        for j in range(0, 3):
            ...
            if POLE_GRY[pole] == 1:
                # rysowanie kółka  gracza
                prost_kol = pygame.draw.rect(oknogry,(0,0,0),[50,-50,50,-50])
                kolko = pygame.image.load("kolko.png")
                prost_kol.center = (x-25,y-25)
                oknogry.blit(kolko,prost_kol)
            elif POLE_GRY[pole] == 2:
                # rysowanie krzyżyka przez komputer
                prost = pygame.draw.rect(oknogry, (255, 0, 0), [50,-50,50,-50])
                krzyzyk = pygame.image.load("krzyzyk.png")
                prost.center = (x-25,y-25)
                oknogry.blit(krzyzyk,prost)



# stawianie kółka lub krzyżka
...
# funkcja pomocnicza sprawdzająca, czy komputer może wygrać, czy powinien blokować gracza, czy może wygrał komputer lub gracz
def sprawdz_pola(uklad, wygrany=None):
    wartosc = None
    # lista wielowymiarowa
    # w Pythonie jest to tablica składająca się z mniejszych tablic.
    ...
    for lista in POLA_INDEKSY:
        ...
        if (kol in uklad):  # jeżeli znalazłeś układ wygrywający lub blokujący zwróć wygranego (1,2) lub indeks pola do zaznaczenia
...
# sprawdź, kto wygrał, a może remis?
def kto_wygral():
...
def drukuj_wynik(WYGRANY):
    ...
    if WYGRANY == 1:
        ...
        tekst = 'Remis!'
    tekst_obr = fontObj.render(tekst, True, (0, 255, 234))
    tekst_prost = tekst_obr.get_rect()
    tekst_prost.center = (150, 150)
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if WYGRANA is False: #wartość True, która włącza grę
            if RUCH == 1:
